...
Shader::~Shader ()
{
  ...
  std::cout << "Deleted shader : " << this << std::endl;
}
...
void
Shader::setVec4 (const std::string &name, float x, float y, float z,
                 float w) const
{
...
// ------------------------------------------------------------------------
void
Shader::checkCompileErrors (GLuint shader, std::string type)
{
  GLint success;
  GLchar infoLog[1024];
  if (type != "PROGRAM")
